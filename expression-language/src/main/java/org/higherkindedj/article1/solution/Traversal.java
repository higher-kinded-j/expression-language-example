 ...
 *
 * <p>Unlike a Lens which focuses on exactly one value, a Traversal
 * can focus on multiple values simultaneously - perfect for updating
 * all elements in a collection.</p>
 *
 * <p>This is a simplified implementation demonstrating the concept.</p>
 *
...
public record Traversal<S, A>(
    ...
    java.util.function.BiFunction<Function<A, A>, S, S> modifyAll
) {
    ...
    public S set(A newValue, S whole) {
        return modify(ignored -> newValue, whole);
    }
     ...
     */
    public static <S, A, B> Traversal<S, B> fromLens(Lens<S, List<A>> lens, Traversal<List<A>, A> listTraversal, Lens<A, B> elemLens) {
        return new Traversal<>(
            s -> {
